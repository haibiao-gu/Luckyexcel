import { appFile, calcChainFile, coreFile, numFmtDefault, numFmtDefaultMap, sharedStringsFile, stylesFile, theme1File, workBookFile, workbookRels } from "../common/constant";
...
export class LuckyFile extends LuckyFileBase {
  ...
  constructor(files: IuploadfileList, fileName: string, imageList: ImageList) {
    super();
    this.files = files;
  ...
  static async createAsync(files: IuploadfileList, fileName: string, imageHandler?: (imageData: string | ArrayBuffer, fileName: string) => Promise<string>): Promise<LuckyFile> {
    const readXml = new ReadXml(files);
    const sheetNameList = LuckyFile.getSheetNameListStatic(readXml);

    const sharedStrings = readXml.getElementsByTagName("sst/si", sharedStringsFile);
    const calcChain = readXml.getElementsByTagName("calcChain/c", calcChainFile);

    const styles: IStyleCollections = {};
    styles["cellXfs"] = readXml.getElementsByTagName("cellXfs/xf", stylesFile);
    styles["cellStyleXfs"] = readXml.getElementsByTagName("cellStyleXfs/xf", stylesFile);
    styles["cellStyles"] = readXml.getElementsByTagName("cellStyles/cellStyle", stylesFile);
    styles["fonts"] = readXml.getElementsByTagName("fonts/font", stylesFile);
    styles["fills"] = readXml.getElementsByTagName("fills/fill", stylesFile);
    styles["borders"] = readXml.getElementsByTagName("borders/border", stylesFile);
    styles["dxfs"] = readXml.getElementsByTagName("dxfs/dxf", stylesFile);
    styles["clrScheme"] = readXml.getElementsByTagName("a:clrScheme/a:dk1|a:lt1|a:dk2|a:lt2|a:accent1|a:accent2|a:accent3|a:accent4|a:accent5|a:accent6|a:hlink|a:folHlink", theme1File);
    styles["indexedColors"] = readXml.getElementsByTagName("colors/indexedColors/rgbColor", stylesFile);
    styles["mruColors"] = readXml.getElementsByTagName("colors/mruColors/color", stylesFile);

    const imageList = await ImageList.createAsync(files, imageHandler);

    let numfmts = readXml.getElementsByTagName("numFmt/numFmt", stylesFile);
    let numFmtDefaultC = JSON.parse(JSON.stringify(numFmtDefault));
    for (let i = 0; i < numfmts.length; i++) {
      let attrList = numfmts[i].attributeList;
      let numfmtid = getXmlAttibute(attrList, "numFmtId", "49");
    ...
    styles["numfmts"] = numFmtDefaultC;

    const instance = new LuckyFile(files, fileName, imageList);
    instance.readXml = readXml;
    instance.sharedStrings = sharedStrings;
    instance.calcChain = calcChain;
    instance.styles = styles;
    instance.sheetNameList = sheetNameList;

    return instance;
  }

  /**
   * Static method to get sheet name list
   */
  private static getSheetNameListStatic(readXml: ReadXml): IattributeList {
    let workbookRelList = readXml.getElementsByTagName("Relationships/Relationship", workbookRels);
